...
void custom::begin_child( const char* name, ImVec2 size ) {
    ...
    GetWindowDrawList( )->AddText( GetIO( ).Fonts->Fonts[3], GetIO( ).Fonts->Fonts[0]->FontSize, { p.x + 15, p.y + 15 }, GetColorU32( ImGuiCol_Text ), name );
    SetCursorPosY( 50 );
...
bool custom::collapse_button( bool collapsed ) {
    ...
    ImRotateStart( );
    RenderArrow( window->DrawList, bb.GetCenter( ) - ImVec2( 4 + 4 * !anim, 11 ), GetColorU32( ImGuiCol_TextDisabled ), ImGuiDir_Down, 15.f );
